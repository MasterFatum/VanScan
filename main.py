...
def scan_ip_range_selected_ports(ip_range, port_list):
    try:
        network = ipaddress.IPv4Network(ip_range, strict=False)
        print(f"\nScanning network: {network}\n")
...
def main():
    ...
    choice = input("Choose mode:\n1 - Scan single host (ALL PORTS)\n2 - Scan IP range (ALL PORTS)\n3 - Scan IP range (SELECTED PORT)\n4 - Exit\n> ")

    match choice:
        case "1":
            host = input('Enter the host name or IP address (ALL PORTS): ')
            if host:
                scan_port(host, ports)

        case "2":
            ip_range = input("Enter IP range (CIDR format, EXAMPLE: 192.168.1.0/24): ")
            scan_ip_range(ip_range, ports)
        case "3":
            ip_range = input("Enter IP range (CIDR format, EXAMPLE: 192.168.1.0/24): ")
            ports_input = input("Enter ports (EXAMPLE: 22, 23, 3306): ")
            try:
                selected_ports = [int(port.strip()) for port in ports_input.split(',') if port.strip().isdigit()]
                if not selected_ports:
                    print("No valid ports entered.")
                else:
                    scan_ip_range_selected_ports(ip_range, selected_ports)
            except ValueError as e:
                print("Invalid port input. Please enter numeric values separated by commas.")
        case "4":
            exit(0)
